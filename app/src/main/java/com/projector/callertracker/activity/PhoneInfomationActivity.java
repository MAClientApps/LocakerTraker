...
public class PhoneInfomationActivity extends BaseActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        deviceSize = Math.sqrt(((((float) this.dm.widthPixels) / this.dm.xdpi) * (((float) this.dm.widthPixels) / this.dm.xdpi)) + ((((float) this.dm.heightPixels) / this.dm.ydpi) * (((float) this.dm.heightPixels) / this.dm.ydpi)));
    ...
    public void onResume() {
        ...
        if (this.deviceName != null) {
            if (ContextCompat.checkSelfPermission(PhoneInfomationActivity.this, Manifest.permission.BLUETOOTH_CONNECT) == PackageManager.PERMISSION_DENIED) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                    ActivityCompat.requestPermissions(PhoneInfomationActivity.this, new String[]{Manifest.permission.BLUETOOTH_CONNECT}, 222);
                    return;
                }
            } else {
                this.deviceName.setText(getPhoneName());
    ...
    public String getCameraMegaPixels(int val) {
        ...
        if (!(arrayListForWidth.size() == 0 || arrayListForHeight.size() == 0)) {
            ...
            wid = Collections.max(arrayListForWidth).intValue();
            heigh = Collections.max(arrayListForHeight).intValue();
        }
